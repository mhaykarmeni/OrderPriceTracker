...
void run_workload(OrderBook& book, const std::vector<Operation>& ops) {
...
static RegisterBenchmark reg_solution(
    "BM_Solution", 100'000,
    [](int iterations) -> uint64_t {
        ...
        for (int i = 0; i < iterations; ++i) {
            OrderBook book;
            uint64_t start = cycle_start();
            run_workload(book, ops);
            clobber();
            uint64_t end = cycle_end();
